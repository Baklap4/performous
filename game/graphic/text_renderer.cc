...
OpenGLText TextRenderer::render(std::string const& text, TextStyle const& style, float m) {
    ...
    cairo_set_antialias(dc.get(), CAIRO_ANTIALIAS_BEST);
    cairo_push_group_with_content(dc.get(), CAIRO_CONTENT_COLOR_ALPHA);
    cairo_set_operator(dc.get(), CAIRO_OPERATOR_SOURCE);

    // Apply Drop Shadow if the shadow color alpha is not zero
    if (style.shadow_col.a > 0.0f) {
        // Set shadow color and opacity
        cairo_set_source_rgba(dc.get(), style.shadow_col.r, style.shadow_col.g, style.shadow_col.b, style.shadow_col.a);

        // Set shadow offset (you can tweak these values)
        float shadow_offset_x = style.shadow_offset_x;  // Use parsed shadow offset
        float shadow_offset_y = style.shadow_offset_y;  // Use parsed shadow offset

        // Draw the shadow first
        cairo_move_to(dc.get(), 0.5f * border + shadow_offset_x, 0.5f * border + shadow_offset_y);  // Adjust for shadow offset
        pango_cairo_update_layout(dc.get(), layout.get());
        pango_cairo_layout_path(dc.get(), layout.get());
        cairo_fill_preserve(dc.get());  // Use the shadow color for filling
    }

    cairo_new_path(dc.get());
